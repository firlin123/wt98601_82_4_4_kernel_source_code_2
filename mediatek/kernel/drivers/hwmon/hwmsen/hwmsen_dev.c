...
static char gyro_sensor_name[25];
static struct sensor_init_info* gyro_sensor_init_list[MAX_CHOOSE_GYRO_NUM] = {0};
#endif
/*----------------------------------------------------------------------------*/
struct dev_context {
    int		polling_running;
...
int hwmsen_gyro_sensor_remove(struct platform_device *pdev)
{
    //int err =0;
	int i=0;
	for(i = 0; i < MAX_CHOOSE_GYRO_NUM; i++)
	{
	   if(0 ==  strcmp(gyro_sensor_name,gyro_sensor_init_list[i]->name))
	   {
	      if(NULL == gyro_sensor_init_list[i]->uninit)
	      {
	        HWM_LOG(" hwmsen_gyro_sensor_remove null pointer \n");
	        return -1;
	      }
	      gyro_sensor_init_list[i]->uninit();
	   }
	}
    return 0;
}
...
static int gyro_sensor_probe(struct platform_device *pdev) 
{
	...
	for(i = 0; i < MAX_CHOOSE_GYRO_NUM; i++)
	{
	  if(NULL != gyro_sensor_init_list[i])
	  {
	    err = gyro_sensor_init_list[i]->init();
		if(0 == err)
		{
		   strcpy(gyro_sensor_name,gyro_sensor_init_list[i]->name);
...
static struct platform_driver gyro_sensor_driver = {
	...
	.remove     = hwmsen_gyro_sensor_remove,    
	.driver     = 
	{
		.name  = "gyroscope",
//		.owner = THIS_MODULE,
...
int hwmsen_gyro_sensor_add(struct sensor_init_info* obj) 
{
    int err=0;
	int i =0;
	...
	for(i =0; i < MAX_CHOOSE_GYRO_NUM; i++ )
	{
	    if(NULL == gyro_sensor_init_list[i])
	    {
	          gyro_sensor_init_list[i] = kzalloc(sizeof(struct sensor_init_info), GFP_KERNEL);
		  if(NULL == gyro_sensor_init_list[i])
		  {
		     HWM_ERR("kzalloc error");
		     return -1;
...
EXPORT_SYMBOL_GPL(hwmsen_gyro_sensor_add);
