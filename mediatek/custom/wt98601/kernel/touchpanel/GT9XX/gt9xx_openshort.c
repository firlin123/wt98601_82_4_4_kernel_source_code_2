...
u16 max_limit_value = 3250;     // screen max limit
u16 min_limit_value = 1300;     // screen min limit
u16 max_limit_key = 1200;       // key_val max limit
...
#if (GTP_GESTURE_WAKEUP||GTP_GROVE_GROUP_DETECT)
struct kobject *goodix_grove_gesture_kobj;
#endif
...
static char grove_value = 0;
static char gesture_value = 0;
char gescontext = 0;
int tp_gesture_onoff = 0;
int tp_grove_onoff = 0;

static ssize_t gtp_sysfs_grove_show(struct device *dev,struct device_attribute *attr, char *buf)
{		
	return sprintf(buf,"%c\n",grove_value);
}

static ssize_t gtp_sysfs_grove_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	if(buf != NULL && count != 0)
	{
		grove_value = buf[0];		
		...
		{
			tp_grove_onoff=1;
			printk("grove_value=1\n");
		}
		else if(buf[0]== '0')
		{
			tp_grove_onoff=0;
			printk("grove_value=0\n");
		}
...
static ssize_t gtp_sysfs_gesture_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	return sprintf(buf,"%c\n",gesture_value);
}

static ssize_t gtp_sysfs_gesture_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	if(buf != NULL && count != 0)
	{
		gesture_value = buf[0];		
		...
		{
			tp_gesture_onoff = 1;
			printk("gesture_value=1\n");
		}
		else if(buf[0]== '0')
		{
			tp_gesture_onoff = 0;
			printk("gesture_value=0\n");
		}
	}
	return count;
}
static ssize_t gtp_sysfs_gescont_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	return sprintf(buf,"%c\n",gescontext);
}

static ssize_t gtp_sysfs_gescont_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	return -1;
}

static DEVICE_ATTR(grove, S_IRUGO|S_IWUGO, gtp_sysfs_grove_show, gtp_sysfs_grove_store);
static DEVICE_ATTR(gesture, S_IRUGO|S_IWUGO, gtp_sysfs_gesture_show, gtp_sysfs_gesture_store);
static DEVICE_ATTR(gescont, S_IRUGO|S_IWUSR, gtp_sysfs_gescont_show, gtp_sysfs_gescont_store);

s32 gtp_grove_gesture_sysfs_init(void)
{
    s32 ret ;
    
    goodix_grove_gesture_kobj = kobject_create_and_add("gtp_grove_gesture", NULL) ;
    SET_INFO_LINE_INFO("Starting gtp_grove_gesture_sysfs_init");
    if (goodix_grove_gesture_kobj == NULL)
    {
        GTP_ERROR("%s: subsystem_register failed\n", __func__);
        return -1;
    }

    ret = sysfs_create_file(goodix_grove_gesture_kobj, &dev_attr_grove.attr);
    if (ret)
    {
        GTP_ERROR("%s: sysfs_create_grove_file failed\n", __func__);
        return ret;
    }
    ret = sysfs_create_file(goodix_grove_gesture_kobj, &dev_attr_gesture.attr);
    if (ret)
    {
        GTP_ERROR("%s: sysfs_create_gesture_file failed\n", __func__);
        return ret;
    }
    ret = sysfs_create_file(goodix_grove_gesture_kobj, &dev_attr_gescont.attr);
    if (ret)
    {
        GTP_ERROR("%s: sysfs_create_gescont_file failed\n", __func__);
        return ret;
    }

    GTP_INFO("Goodix gtp sysfs create success!\n");
    return 0 ;

}

void gtp_grove_gesture_sysfs_deinit(void)
{
    sysfs_remove_file(goodix_grove_gesture_kobj, &dev_attr_grove.attr);
    sysfs_remove_file(goodix_grove_gesture_kobj, &dev_attr_gesture.attr);
    kobject_del(goodix_grove_gesture_kobj);
}
#endif
