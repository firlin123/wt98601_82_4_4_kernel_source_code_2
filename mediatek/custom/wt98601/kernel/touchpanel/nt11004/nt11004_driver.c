...
void nvt_esd_auto_check()
{
	...
	i2c_write_bytes(i2c_client, 0x8C, &buf_send[0], 2);
	msleep(20);
	i2c_read_bytes(i2c_client, 0x8C, &buf_recv[0], 2);
...
static void tpd_down(int id, int x, int y, int w, int press)
{
...
static void tpd_up(int id, int x, int y, int w, int press)
{
...
static int __devinit tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    msleep(1000);
	{
	  ...
	  hardwareinfo_set_prop(HARDWARE_TP, "nt11004");
	}
    tpd_load_status = 1;
...
void tpd_resume(struct i2c_client *client)
{
	...
	mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);//this should be last execubled
}
