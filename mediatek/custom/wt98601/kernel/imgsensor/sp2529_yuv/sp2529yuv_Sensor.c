...

{
    ...
    do {
        *sensorID=((SP2529_read_cmos_sensor(0x02)<< 8)|SP2529_read_cmos_sensor(0xa0));
        if (*sensorID == SP2529_SENSOR_ID)
            break; 
...
UINT32 SP2529Open(void)
{
	...
	for(i=0;i<3;i++)
	{
		sensor_id = (SP2529_read_cmos_sensor(0x02) << 8) | SP2529_read_cmos_sensor(0xa0);
