			...
			u8 tx, u8 rx)
{
	...
	/*scan*/
	err = fts_read_reg(client, FTS_DEVICE_MODE_REG, &regvalue);
	if (err < 0) {
		return err;
	} else {
		regvalue |= 0x80;
		err = fts_write_reg(client, FTS_DEVICE_MODE_REG, regvalue);
		if (err < 0) {
			return err;
		} else {
			for(i=0; i<20; i++)
			{
				msleep(8);
				err = fts_read_reg(client, FTS_DEVICE_MODE_REG, 
							&regvalue);
				if (err < 0) {
					return err;
				} else {
...
u16 maxdata[FTS_TX_MAX][FTS_RX_MAX];
u16 mindata[FTS_TX_MAX][FTS_RX_MAX];
static ssize_t ft5x0x_lcdnoisecheck_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	...
	u8 rx = 0, tx = 0;
	u8 i = 0, j = 0, m=0, n=0;
	int err = 0, testpass = 1;

	mutex_lock(&g_device_mutex);
	/*entry factory*/
	err = fts_write_reg(client, FTS_DEVICE_MODE_REG, FTS_FACTORYMODE_VALUE);
	if (err < 0) {
		num_read_chars = sprintf(buf,
			"%s:rawdata show error!\n", __func__);
			testpass = 0;
		goto RETURN_WORK_MODE;
	}
	mdelay(200);
	/*get rx and tx num*/
	err = fts_read_reg(client, FTS_TXNUM_REG, &tx);
	if (err < 0) {
		num_read_chars = sprintf(buf,
				"%s:get tx error!\n", __func__);
				testpass = 0;
	...
	err = fts_read_reg(client, FTS_RXNUM_REG, &rx);
	if (err < 0) {
		num_read_chars = sprintf(buf,
			"%s:get rx error!\n", __func__);
			testpass = 0;
	...
	for (i=0; i<10; i++) {
		...
		for(m=0; m<tx; m++) {
				for(n=0; n<rx; n++) {
					if (rawdata[m][n] > maxdata[m][n])
						maxdata[m][n] = rawdata[m][n];
					
					if (rawdata[m][n] < mindata[m][n])
						mindata[m][n] = rawdata[m][n];
				}
			}
	} 
...
RETURN_WORK_MODE:
	/*enter work mode*/
	err = fts_write_reg(client, FTS_DEVICE_MODE_REG, FTS_WORKMODE_VALUE);
	if (err < 0)
		dev_err(&client->dev,
			"%s:enter work error!\n", __func__);
	msleep(100);
...
static ssize_t ft5x0x_lcdnoisecheck_store(struct device *dev,
					struct device_attribute *attr,
						const char *buf, size_t count)
{
	/* place holder for future use */
...
static ssize_t ft5x0x_calibrationcheck_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	ssize_t num_read_chars = 0;
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	...
	/*change modify register*/
	
	err = fts_write_reg(client, 0xEA, 0);
	if (err < 0) {
		num_read_chars += sprintf(buf,
			"%s:write 0 to modify register error!\n", __func__);
		testpass = 0;
		goto RETURN_WORK_MODE;
	}
	...
	err = fts_write_reg(client, 0xEA, 1);
	if (err < 0) {
		num_read_chars += sprintf(buf,
			"%s:write 1 to modify register error!\n", __func__);
		testpass = 0;
		goto RETURN_WORK_MODE;
	}
	/*check*/
	for (i=0; i<rx; i++) {
			...
			//判断是否是虚拟按键所在的RX位置，如果不是，则TX循环数-1。
			if (i==2 || i==6 || i==9)
				txloop = tx;
			else
				txloop = tx - 1;
...
//upgrade from app.bin
static ssize_t ft5x0x_calibrationcheck_store(struct device *dev,
					struct device_attribute *attr,
						const char *buf, size_t count)
{
	/* place holder for future use */
...
static DEVICE_ATTR(ftslcdnoisecheck, S_IRUGO | S_IWUSR, ft5x0x_lcdnoisecheck_show, ft5x0x_lcdnoisecheck_store);

static DEVICE_ATTR(ftscalibrationcheck, S_IRUGO | S_IWUSR, ft5x0x_calibrationcheck_show, ft5x0x_calibrationcheck_store);
...
static struct attribute *fts_attributes[] = {
	...
	&dev_attr_ftslcdnoisecheck.attr,
    &dev_attr_ftscalibrationcheck.attr,
	NULL
};
