...
static int tpd_resume_flag = 1;
static u8 charger_jump_algorithm_on = 0; //0:off   1:on
static u8 algorithm_on_tmp = 0;
//extern kal_bool current_charger_is_exist(void);
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
	...
	if(tpd_resume_flag)
	{	
		charger_jump_algorithm_on = 1;//(u8)current_charger_is_exist();
		if(algorithm_on_tmp != charger_jump_algorithm_on)
		{	
			algorithm_on_tmp = charger_jump_algorithm_on;
			i2c_smbus_write_i2c_block_data(i2c_client, 0x8B, 1, &algorithm_on_tmp);
		}
	}
	...
	i2c_smbus_read_i2c_block_data(i2c_client, 0x3c, 8, &(data[60]));
	i2c_smbus_read_i2c_block_data(i2c_client, 0xa6, 1, &(data[61]));
...
{	 
	...
	static char fw_buf[10]={0};
	static char ft5316_info[25]="ft5316  FW:";
...
reset_proc:   
	...
	//mt65xx_eint_set_sens(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINTF_TRIGGER_LOW);
	...
	//---------read TP firmware-----------
	i2c_smbus_read_i2c_block_data(i2c_client, 0xa6, 1, &fw_version);
	sprintf(fw_buf, "0x%x", fw_version);
	//---------read TP firmware-----------
#if 0
	{
		#include <linux/hardware_info.h>
		strcat(ft5316_info, fw_buf);
		hardwareinfo_set_prop(HARDWARE_TP, ft5316_info);
	}
#endif
	tpd_load_status = 1;
...
static int tpd_local_init(void)
{
 ...
 static void tpd_resume( struct early_suspend *h )
 {
  //int retval = TPD_OK;
  //char data;
  u8 charger_jump_algorithm_value;
	...
	tpd_resume_flag = 1;
	i2c_smbus_read_i2c_block_data(i2c_client, 0x8B, 1, &charger_jump_algorithm_value);
	algorithm_on_tmp = charger_jump_algorithm_value;
