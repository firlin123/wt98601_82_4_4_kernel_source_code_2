...
static int lsm330_local_init(void);
static int lsm330_remove(void);
static int LSM330_remap_to_gravity(struct i2c_client *client, int data[LSM330_AXES_NUM]);
static int lsm330_init_flag = -1; // 0<==>OK , -1 <==> fail
static struct sensor_init_info lsm330_init_info = {
		.name 	= LSM330_DEV_NAME,
		.init 	= lsm330_local_init,
		.uninit = lsm330_remove,
};
...
*/
/*----------------------------------------------------------------------------*/
static int LSM330_WriteCalibration_New(struct i2c_client *client, struct GS_CALI_DATA_STRUCT *data_cali)
{
	struct lsm330_i2c_data *obj = i2c_get_clientdata(client);
...
static int LSM330_WriteCalibration(struct i2c_client *client, int dat[LSM330_AXES_NUM])
{
	...
	{ 
		...
		obj->cali_sw[LSM330_AXIS_X] = obj->cvt.sign[LSM330_AXIS_X]*(cali[obj->cvt.map[LSM330_AXIS_X]]);
		obj->cali_sw[LSM330_AXIS_Y] = obj->cvt.sign[LSM330_AXIS_Y]*(cali[obj->cvt.map[LSM330_AXIS_Y]]);
		obj->cali_sw[LSM330_AXIS_Z] = obj->cvt.sign[LSM330_AXIS_Z]*(cali[obj->cvt.map[LSM330_AXIS_Z]]);
	} 
	return err;
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
static int LSM330_ReadRawData_New(struct i2c_client *client, int buf[3])
{
	struct bma255_i2c_data *obj = (struct bma255_i2c_data*)i2c_get_clientdata(client);
	int res = 0;
	s16 databuf[LSM330_AXES_NUM];
	...
	if(res = LSM330_ReadData(client, databuf))
	{        
...
static int LSM330_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	else
	{
		...
		if(gs_cali.valid == 1){
			acc[LSM330_AXIS_X] -= gs_cali.x;
			acc[LSM330_AXIS_Y] -= gs_cali.y;
			acc[LSM330_AXIS_Z] -= gs_cali.z;
		}
...
/*----------------------------------------------------------------------------*/
static int lsm330_gsensor_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
static int LSM330_remap_to_gravity(struct i2c_client *client, int data[LSM330_AXES_NUM])
{
	...
	/*remap coordinate*/
	acc_tmp[obj->cvt.map[LSM330_AXIS_X]] = obj->cvt.sign[LSM330_AXIS_X]* data[LSM330_AXIS_X];
	acc_tmp[obj->cvt.map[LSM330_AXIS_Y]] = obj->cvt.sign[LSM330_AXIS_Y]* data[LSM330_AXIS_Y];
	acc_tmp[obj->cvt.map[LSM330_AXIS_Z]] = obj->cvt.sign[LSM330_AXIS_Z]* data[LSM330_AXIS_Z];

	//Out put the mg
	data[LSM330_AXIS_X] = acc_tmp[LSM330_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[LSM330_AXIS_Y] = acc_tmp[LSM330_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[LSM330_AXIS_Z] = acc_tmp[LSM330_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_READ_OFFSET:
			...
			case GSENSOR_SET_ACC_MODE:
				...
				if(enable == 1){
					err = LSM330_SetPowerMode(client,true);
				}else{
					err = LSM330_SetPowerMode(client,false);
				}
				if(err)
			...
			case GSENSOR_IOCTL_SET_CALI_NEW: 
				...
				if(atomic_read(&obj->suspend)){
					...
					err = LSM330_WriteCalibration_New(client, &gs_cali_temp);
					if(err){
						err = -EFAULT;
						break;
					}
		...
		case GSENSOR_IOCTL_READ_RAW_DATA:
			...
			*/
			data = (void __user *) arg;
			int raw_data[LSM330_AXES_NUM]={0,0,0};
			int acc[LSM330_AXES_NUM]={0,0,0};
			if(data == NULL)
			{
				err = -EINVAL;
				break;	  
			}
			err = LSM330_ReadRawData_New(client, raw_data);
			if(err){
				err = -EFAULT;
				break;
			}
			err = LSM330_remap_to_gravity(client, raw_data);
			if(err){
				err = -EFAULT;
				break;
			}
			sensor_data.x = raw_data[0];
			sensor_data.y = raw_data[1];
			sensor_data.z = raw_data[2];
			
			err = copy_to_user(data, &sensor_data, sizeof(sensor_data));
			if(err){
				err = -EFAULT;
				break;
			}
...
static int lsm330_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(err = lsm330_create_attr(&(lsm330_init_info.platform_diver_addr->driver)))
	{
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = lsm330_gsensor_operate;
...

	{
		...
		//hardwareinfo_set_prop(HARDWARE_ACCELEROMETER, "lsm330");
	}
...
static int lsm330_i2c_remove(struct i2c_client *client)
{
	...
	if((err = lsm330_delete_attr(&(lsm330_init_info.platform_diver_addr->driver))))
	{
...
static int lsm330_remove(void)
{
    struct acc_hw *hw = lsm330_get_cust_acc_hw();
...
#endif
	hwmsen_gsensor_add(&lsm330_init_info);
