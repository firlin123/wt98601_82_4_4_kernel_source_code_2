...
static int bmi058_a_local_init(void);
static int bmi058_a_remove(void);
static int bmi058_remap_to_gravity(struct i2c_client *client, int data[BMA255_AXES_NUM]);
static int bmi058_a_init_flag = -1; // 0<==>OK , -1 <==> fail
static struct sensor_init_info bmi058_a_init_info = {
		.name 	= BMA255_DEV_NAME,
		.init 	= bmi058_a_local_init,
		.uninit = bmi058_a_remove,
};
/*----------------------------------------------------------------------------*/
typedef enum {
    BMA_TRC_FILTER  = 0x01,
...
static struct data_resolution bma255_data_resolution[1] = {
 /* combination by {FULL_RES,RANGE}*/
    {{ 0, 2}, 4096},   // dataformat +/-2g  in 12-bit resolution;  { 1, 0} = 1.0= (2*2*1000)/(2^12);  1024 = (2^12)/(2*2)          
};
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
static int BMI058_WriteCalibration_New(struct i2c_client *client, struct GS_CALI_DATA_STRUCT *data_cali)
{
	struct bma255_i2c_data *obj = i2c_get_clientdata(client);
...
static int BMA255_CompassReadData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	{
		databuf[BMA255_AXIS_X] >>= 2;
		databuf[BMA255_AXIS_Y] >>= 2;
		databuf[BMA255_AXIS_Z] >>= 2;
		/*remap coordinate*/
...
static int BMA255_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	{		
		...
		if(gs_cali.valid == 1){
			acc[BMA255_AXIS_X] -= gs_cali.x;
			acc[BMA255_AXIS_Y] -= gs_cali.y;
			acc[BMA255_AXIS_Z] -= gs_cali.z;
		}
...
/*----------------------------------------------------------------------------*/
static int bmi058_gsensor_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
static int bmi058_remap_to_gravity(struct i2c_client *client, int data[BMA255_AXES_NUM])
{
	...
	/*remap coordinate*/
	acc_tmp[obj->cvt.map[BMA255_AXIS_X]] = obj->cvt.sign[BMA255_AXIS_X]* data[BMA255_AXIS_X];
	acc_tmp[obj->cvt.map[BMA255_AXIS_Y]] = obj->cvt.sign[BMA255_AXIS_Y]* data[BMA255_AXIS_Y];
	acc_tmp[obj->cvt.map[BMA255_AXIS_Z]] = obj->cvt.sign[BMA255_AXIS_Z]* data[BMA255_AXIS_Z];

	//Out put the mg
	data[BMA255_AXIS_X] = acc_tmp[BMA255_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[BMA255_AXIS_Y] = acc_tmp[BMA255_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[BMA255_AXIS_Z] = acc_tmp[BMA255_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
...
static long bma255_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	//-----------------------------
			...
			case GSENSOR_IOCTL_SET_CALI_NEW:    // yangbo add2
				data = (void __user *)arg;
				if (data == NULL) {
					err = -EINVAL;
					break;
				...
				if(atomic_read(&obj->suspend)){
					...
					err = BMI058_WriteCalibration_New(client, &gs_cali_temp);
					if(err){
						err = -EFAULT;
						break;
					}
	...
	//-----------------------------
		...
		case GSENSOR_IOCTL_READ_RAW_DATA:   // yangbo add 4
		/*	data = (void __user *) arg;
			if(data == NULL)
			{
				err = -EINVAL;
			...
			err = bmi058_remap_to_gravity(client, raw_data);
			if(err){
				err = -EFAULT;
				break;
			}
...
static int bma255_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = bmi058_a_get_cust_acc_hw();
	...
	if(err = bma255_create_attr(&(bmi058_a_init_info.platform_diver_addr->driver)))
	{
	...
	sobj.self = obj;
	sobj.polling = 1;
	sobj.sensor_operate = bmi058_gsensor_operate;
...
#endif 
	{
		...
		//hardwareinfo_set_prop(HARDWARE_ACCELEROMETER, "bmi058");
	}

	GSE_LOG("%s: OK\n", __func__);

	bmi058_a_init_flag = 0;
	return 0;

	exit_create_attr_failed:
		bma255_delete_attr(&(bmi058_a_init_info.platform_diver_addr->driver));
...
static int bma255_i2c_remove(struct i2c_client *client)
{
	...
	if(err = bma255_delete_attr(&(bmi058_a_init_info.platform_diver_addr->driver)))
	{
...
static int bmi058_a_remove(void)
{
    struct acc_hw *hw = bmi058_a_get_cust_acc_hw();
...
#endif
	hwmsen_gsensor_add(&bmi058_a_init_info);
