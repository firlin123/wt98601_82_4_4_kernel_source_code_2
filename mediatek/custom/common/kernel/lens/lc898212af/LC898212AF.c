...
static unsigned long g_release_macro = 0;
...
u16 Hall_Max=0x7FFF;     		//Infinity position
u16 Hall_Min=0x8001; 	   		//Macro position
static u16 Hall_Macro_Pos=0;     		//Macro position
static u16 Hall_Infinity_Pos=0; 	   		//Infinity position
...
u16 AF_convert_1(u16 position)// 10bit to 16bit complement
{
	...
	if(position<=511) 
	{
		position=(((Max_Pos - Min_Pos)/2) - position)*((Hall_Max+(0xFFFF-Hall_Min))/2)/((Max_Pos - Min_Pos)/2);
	}
	else
	{
		position=(((Max_Pos - Min_Pos)/2)- position)*((Hall_Max+(0xFFFF-Hall_Min))/2)/(((Max_Pos - Min_Pos)/2)+1);
		position = ~((position*-1))+1;
	}
...
u16 AF_convert_2(u16 position)// 16bit complement to 10 bit
{
	...
	if(position>0x7FFF) //0xFFFF -0x8001
	{
		temp = position | 0xFFFF0000;
		LC898212AFDB("[LC898212AF] AF_convert 2: temp 1=%d\n",temp);
		position = ((Max_Pos - Min_Pos)/2) - temp*(((Max_Pos - Min_Pos)/2)+1)/((Hall_Max+(0xFFFF-Hall_Min))/2);
	}
	else //0x7FFF - 0
	{
		position = ((Max_Pos - Min_Pos)/2) - position*((Max_Pos - Min_Pos)/2)/((Hall_Max+(0xFFFF-Hall_Min))/2);
	}
...
static void readCalibrationData()
{
...
#endif	
	eeprom_reg_read_data1=s4AF_EEPROM_Reg_ReadA8(0x03);
	eeprom_reg_read_data2=s4AF_EEPROM_Reg_ReadA8(0x04);
	g_release_macro=((eeprom_reg_read_data1<<8) | eeprom_reg_read_data2);
	...
	LC898212AFDB("[LC898212AF] readCalibrationData:g_release_inf=%x,g_release_macro=%x\n",
		g_release_inf,g_release_macro);

	g_release_inf=AF_convert_2(g_release_inf);
	g_release_macro=AF_convert_2(g_release_macro);

	//g_u4LC898212AF_INF = g_release_inf;
	//g_u4LC898212AF_MACRO = g_release_macro;
	...
	Hall_Infinity_Pos=(eeprom_reg_read_data1<<8) | eeprom_reg_read_data2;
	Hall_Max=0x7FFF;
	
	LC898212AFDB("[LC898212AF] readCalibrationData:Hall_Infinity_Pos =%x\n",Hall_Infinity_Pos);


	eeprom_reg_read_data1=s4AF_EEPROM_Reg_ReadAA(0x07/*0x08*//*0x04*/);
	eeprom_reg_read_data2=s4AF_EEPROM_Reg_ReadAA(0x08/*0x09*//*0x05*/);
	Hall_Macro_Pos=(eeprom_reg_read_data1<<8) | eeprom_reg_read_data2;
	Hall_Min=0x8001;

	LC898212AFDB("[LC898212AF] readCalibrationData:Hall_Macro_Pos =%x\n",Hall_Macro_Pos);
	Hall_Infinity_Pos=AF_convert_2(Hall_Infinity_Pos);
	Hall_Macro_Pos=AF_convert_2(Hall_Macro_Pos);
	LC898212AFDB("[LC898212AF] readCalibrationData22:Hall_Macro_Pos=%d,Hall_Infinity_Pos =%d\n",Hall_Macro_Pos,Hall_Infinity_Pos);
...
inline static int moveLC898212AF(unsigned long a_u4Position)
{
    ...
    if (a_u4Position < Hall_Infinity_Pos) 
    {
        g_u4TargetPosition = Hall_Infinity_Pos+5;
    }
    else if (a_u4Position > Hall_Macro_Pos)
    {
        g_u4TargetPosition = Hall_Macro_Pos-5;
    }
...
static int LC898212AF_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    i=0;
    if(AF_convert_2(ram_read_data) > g_release_macro && AF_convert_2(ram_read_data) > 900)
    {
        while(1)
        {
            i++;
...
#ifdef LC898212AF_DEBUG
static ssize_t Store_LC891212_MovePos(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	u16 pos = 0;
	sscanf(buf, "%x", &pos);
	s4LC898212AF_RAM_Write(0x04,pos);
	LC898212AFDB("[LC898212AF] store pos 0x%x\n", pos);
	return size;
}
static DEVICE_ATTR(LC891212_MovePos, 0664, NULL, Store_LC891212_MovePos);
#endif
...
#ifdef LC898212AF_DEBUG
	device_create_file(&(client->dev), &dev_attr_LC891212_MovePos);
#endif
