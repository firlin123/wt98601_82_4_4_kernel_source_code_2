...
#ifdef SUPPORT_IMX135_OTP
extern int iReadIMX135LSCData(unsigned char *pinputdata);
#endif
...
#ifdef SUPPORT_IMX135_OTP
	int i;
	unsigned char lsc_data[504];

	iReadIMX135LSCData(&lsc_data);
	for(i=0;i<504;i++)
	{
	   IMX135MIPI_write_cmos_sensor(0x4800+i,lsc_data[i]);
	   //SENSORDB("IMX135MIPI_globle_setting:%d,0x%x \n",i,lsc_data[i]);
	}

	IMX135MIPI_write_cmos_sensor(0x3A63,0x01);
	IMX135MIPI_write_cmos_sensor(0x4500,0x1F);
	IMX135MIPI_write_cmos_sensor(0x0700,0x01);
	SENSORDB("IMX135MIPI_globle_setting  end \n");
#endif
