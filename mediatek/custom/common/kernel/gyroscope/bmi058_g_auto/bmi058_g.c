...
#if defined(MT6573) || defined(MT6575) || defined(MT6577) || defined(MT6589) || defined(MT6592)|| defined(MT6582)
...
#include "bmi058_g.h"
/*----------------------------------------------------------------------------*/
static int bmi058_g_local_init(void);
static int bmi058_g_remove(void);
static int bmi058_g_init_flag = -1; // 0<==>OK , -1 <==> fail
static struct sensor_init_info bmi058_g_init_info = {
		.name 	= BMG_DEV_NAME,
		.init 	= bmi058_g_local_init,
		.uninit = bmi058_g_remove,
};
/*----------------------------------------------------------------------------*/
...
static int bmi058_gyroscope_operate(void *self, uint32_t command, void *buff_in, int size_in,
		...
		const struct i2c_device_id *id)
{
	...
	obj->hw = bmi058_g_get_cust_gyro_hw();
	...
	err = bmg_create_attr(&(bmi058_g_init_info.platform_diver_addr->driver));
	if (err) {
	...
	sobj.self = obj;
	sobj.polling = 1;
	sobj.sensor_operate = bmi058_gyroscope_operate;
...

	{
		...
		//hardwareinfo_set_prop(HARDWARE_GYROSCOPE, "bmi058");
	}

	GYRO_LOG("%s: OK\n", __func__);
	bmi058_g_init_flag = 0;
	return 0;

exit_hwmsen_attach_failed:
	bmg_delete_attr(&(bmi058_g_init_info.platform_diver_addr->driver));
...
static int bmg_i2c_remove(struct i2c_client *client)
{
	...
	err = bmg_delete_attr(&(bmi058_g_init_info.platform_diver_addr->driver));
	if (err)
...
static int bmi058_g_remove(void)
{
	struct gyro_hw *hw = bmi058_g_get_cust_gyro_hw();
	GYRO_FUN();
...
static int bmi058_g_local_init(void)
{
   	struct acc_hw *hw = bmi058_g_get_cust_gyro_hw();
	GYRO_FUN();
...
#endif
	hwmsen_gyro_sensor_add(&bmi058_g_init_info);
