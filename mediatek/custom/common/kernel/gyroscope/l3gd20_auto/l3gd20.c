...
static int l3gd20_init_flag = -1; // 0<==>OK , -1 <==> fail
static struct sensor_init_info l3gd20_init_info = {
...
/*----------------------------------------------------------------------------*/
static int l3gd20_gyro_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
static int l3gd20_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = l3gd20_gyro_operate;
...

	{
		...
		//hardwareinfo_set_prop(HARDWARE_GYROSCOPE, "lsm330");
	}
	...
	exit_create_attr_failed:
		l3gd20_delete_attr(&(l3gd20_init_info.platform_diver_addr->driver));
...
static int  l3gd20_local_init(void)
{
    struct acc_hw *hw = l3gd20_get_cust_gyro_hw();
	GYRO_FUN();
...
#endif
	hwmsen_gyro_sensor_add(&l3gd20_init_info);
