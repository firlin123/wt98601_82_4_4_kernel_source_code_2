...
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT //Qlw 2013/12/20
        case MD_MAIN_MIC_ACOUSTIC_LOOPBACK:
        {
            AudioAnalogControlExt::getInstance()->SetAnalogGain(AudioAnalogType::VOLUME_IN_FACTORY_MAIN_MIC, true);
            SpeechLoopbackController::GetInstance()->OpenModemLoopbackControlFlow(mWorkingModemIndex, input_device, output_device);
            break;
        }
        case MD_REF_MIC_ACOUSTIC_LOOPBACK:
        {
            AudioAnalogControlExt::getInstance()->SetAnalogGain(AudioAnalogType::VOLUME_IN_FACTORY_REF_MIC, true);
            SpeechLoopbackController::GetInstance()->OpenModemLoopbackControlFlow(mWorkingModemIndex, input_device, output_device);
            break;
        }
        case MD_HEADSET_MIC_ACOUSTIC_LOOPBACK:
...
status_t LoopbackManager::SetLoopbackOff()
{
    ...
    switch (mLoopbackType)
    {
        ...
        case MD_MAIN_MIC_ACOUSTIC_LOOPBACK:
        {
            AudioAnalogControlExt::getInstance()->SetAnalogGain(AudioAnalogType::VOLUME_IN_FACTORY_MAIN_MIC, false);
            SpeechLoopbackController::GetInstance()->CloseModemLoopbackControlFlow(mWorkingModemIndex);
            break;
        }
        case MD_REF_MIC_ACOUSTIC_LOOPBACK:
        {
            AudioAnalogControlExt::getInstance()->SetAnalogGain(AudioAnalogType::VOLUME_IN_FACTORY_REF_MIC, false);
            SpeechLoopbackController::GetInstance()->CloseModemLoopbackControlFlow(mWorkingModemIndex);
            break;
        }
        case MD_HEADSET_MIC_ACOUSTIC_LOOPBACK:
